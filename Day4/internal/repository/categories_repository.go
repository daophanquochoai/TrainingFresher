...
func NewPostgresCategoryRepository(db *sql.DB, redis *redis.Client) CategoryRepository {
...
func (r *PostgresCategoryRepository) CreateCategory(ctx context.Context, category *model.Categories) (*model.Categories, error) {
...
// get category by id
func (r *PostgresCategoryRepository) GetCategoryById(ctx context.Context, categoryId string) (*model.Categories, error) {

	// cache
	key := baseKeyCategory + categoryId
	cached, errCache := r.redis.Get(ctx, key).Bytes()
	if errCache == nil {
		var category model.Categories
		if err := json.Unmarshal(cached, &category); err == nil {
			fmt.Println("INFO: Using category in redis")
			return &category, nil
		}
	}
	...
	row := r.db.QueryRowContext(ctx, quey, categoryId)
	...
	// cache
	fmt.Println("Caching category iin redis")
	b, _ := json.Marshal(category)
	_ = r.redis.Set(ctx, key, b, 60*time.Second)

	return category, err
}

// update category by id
func (r *PostgresCategoryRepository) UpdateCategoryById(ctx context.Context, categoryId string, category *model.Categories) (*model.Categories, error) {
	// query
	query := "SELECT id, name FROM update_category_by_id($1, $2)"
	row := r.db.QueryRowContext(ctx, query, categoryId, category.Name)
...
func (r *PostgresCategoryRepository) DeleteCategoryById(ctx context.Context, categoryId string) error {
	// query
...
func (r *PostgresCategoryRepository) AddProductIntoCategory(ctx context.Context, categoryParam *model.CategoryRequest) (*model.CategoryAndProduct, error) {

	// get info category
	query := "SELECT id, name FROM categories c WHERE c.id = $1"
	rowCate := r.db.QueryRowContext(ctx, query, categoryParam.CategoryId)
	...
	rowCatePro, err := r.db.QueryContext(ctx, queryCatePro, categoryParam.CategoryId)
	if err != nil {
		return nil, err
	}
	...
	for _, pid := range toAdd {
		print("pid :", pid)
		if _, err := tx.ExecContext(ctx, insertQuery, pid, categoryParam.CategoryId); err != nil {
			tx.Rollback()
			return nil, err
		}
	}
	...
	for _, pid := range toRemove {
		if _, err := tx.ExecContext(ctx, deleteQuery, pid, categoryParam.CategoryId); err != nil {
			tx.Rollback()
			return nil, err
		}
	}
	...
	rowProCate, err := r.db.QueryContext(ctx, queryProductCategory, categoryParam.CategoryId)
	if err != nil {
		return nil, err
	}
